# خطة بناء Seven Studios — تطبيق سطح مكتب «محلي أولاً» لتحرير الفيديو وتوليده بالذكاء الاصطناعي

## 0. السياق (لماذا هذه الخطة)

- المستودع `moathkleap/Seven Studios` **فارغ تماماً**: لا commits، لا فروع على GitHub، لا كود، لا إطار عمل، لا قاعدة بيانات، لا واجهة. البناء يبدأ من الصفر على الفرع `claude/ai-video-editor-desktop-app-coa0wr`.
...
| الاسم | اسم العرض **Seven Studios**، معرّف الحزم `sevenstudios` | أسماء الحزم لا يُفضَّل أن تبدأ برقم |
...
**خارج القاعدة (في `data_dir` للمشروع):** `project.sevenstudios.json` (مرآة قابلة للنقل)، `journal.ndjson` (patches تراكمية بين اللقطات للاستعادة)، `tracks/<id>.json` (إطارات الأقنعة/التتبع)، `cache/` (proxies, thumbnails, waveforms, preview renders)، `generated/` (أصول Creator)، `exports/`.
...
- الإطار Electron وليس Tauri (مبرر تقني في §1). الاسم `Seven Studios`/`sevenstudios`.
